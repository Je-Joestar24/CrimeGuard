  ...
  <div class="w-3/4 h-full bg-white p-3 rounded-lg border">
    ...
    <div class="mt-3 gap-10 flex ">
      <div class="flex flex-col justify-start">
        ...
        <svg
          v-if="data.gender == null"          class="w-10 h-10"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
...
export default {
  ...
  created() {
    (async () => {
      const send = await {
        url: "api/citizenUsers/view/basic/details/request",
        data: {
  ...
  methods: {
    async sendData(param) {
      ...
      if (res != "Success") {
        ...
        this.set(this.data['profile'], this.data['user_name'])
