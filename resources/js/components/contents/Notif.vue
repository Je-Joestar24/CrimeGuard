<template>
  <div class="bg-white shadow-lg rounded-lg overflow-hidden">
    <div class="p-6">
      <h2 class="text-2xl font-bold text-gray-800 mb-4">Incident Report Notifications</h2>
          ...
          <li v-for="el in paginatedArr" :key="el.id" class="hover:bg-gray-100 transition duration-150 ease-in-out">
            <div class="p-4 flex items-center space-x-4">
              <div class="flex-shrink-0">
                <div class="h-10 w-10 rounded-full bg-blue-500 flex items-center justify-center text-white font-semibold text-lg">
              ...
              <div class="text-sm text-gray-500">
                <p>{{ el.message }} at {{ el.location }}, {{ el.time }}</p>
              </div>
      ...
      <div v-if="paginatedArr.length === 0" class="text-center py-8">
        <p class="text-gray-500 text-lg">No incident report notifications found</p>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    this.getData({url: "api/notifications/list/Display", data: {search: ""}});
  },
  methods: {
    async getData(param) {
