  ...
  >
    <div class="relative w-full max-w-6xl">
      <div class="relative bg-white rounded-lg border shadow-2xl">
        <div class="flex justify-end p-3 border-b">
          <span class=" w-full px-2 py-2 text-xl font-bold">ADD INCIDENT</span>
          <button
            type="button"
    ...
    <div v-if=" incidentT.list.length > 0" class=" max-h-32 overflow-auto fixed top-72 w-96 bg-white divide-y divide-gray-100 rounded-lg border shadow ">
      <ul class="py-2 text-sm text-gray-700 border-b" >
        <li  v-for="inc in incidentT.list"  ><!-- @click="data.incident_types.incident_name = inc.incident_name" -->
...
export default {
  ...
  methods: {
    ...
    async sendData() {
      ...
      data1.incident["added_by"] = this.id;
      data1["id"] = this.id;
      const send = await {
        data: data1,
        url: "api/incidents/add/item/request",
      };
