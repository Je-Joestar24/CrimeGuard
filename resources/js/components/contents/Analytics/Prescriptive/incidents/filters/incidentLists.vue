<template>
  ...
  <div class="mt-2 max-h-60 overflow-y-auto bg-white rounded-lg shadow-md absolute z-50">
    <ul class="divide-y divide-violet-100">
      <li 
        v-for="inc in incidentT.list" 
        :key="inc.id"
        @click="setIncident(inc.incident_name, inc.id)"
    ...
    <div v-if="incidentT.list.length === 0" class="px-4 py-2 text-violet-500 text-center">
      No incidents found
    </div>
  </div>
</template>
